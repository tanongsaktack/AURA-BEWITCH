...
if password != "1234":  # เปลี่ยน 1234 เป็นรหัสจริง
    st.stop()
...
if st.button("ประมวลผล"):
    selected = [f"ติ๊ก {i}" for i in range(1,5) if locals()[f"opt{i}"].value]
    st.success(f"เลือก: {', '.join(selected) or 'ยังไม่ได้ติ๊ก'}")